...
import test.cn.example.com.androidskill.model.explode.particle.VerticalAscentParticle;

/**
 * Created by xgxg on 2017/8/29.
 */

public class VerticalAscentParticleFactory extends ParticleFactory {
    ...
    public Particle[][] generateParticles(Bitmap bitmap, Rect rect) {
        ...
        for (int row = 0; row < partH_Count; row++) {
            for (int column = 0; column < partW_Count; column++) {
                ...
                particles[row][column] = new VerticalAscentParticle(color,x,y,rect);
            }
        }
        return particles;
    }
